    ...
    return res.json(
      {
        ...
        data: {
          ...
          cards: (await user.getCards()).map(card => ({name: card.cardName,flag: card.cardFlag, number: "**** **** **** " + card.cardNumber.toString().slice(-4), validityYear: moment(card.cardValidity).format("YYYY")}))
        }
      }
    ).end();
  }
